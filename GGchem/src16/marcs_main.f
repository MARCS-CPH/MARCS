...
c     variable declaration for writing of partial pressures
     ...
     >  pp_atoms, pp_molecules, pp_non, pp_sum, total_h
...
c     i.e. the one which marcs is iterating over atm
      ...
      open(unit=321, file='ndensity.dat', status='replace')
...
c      print *, "Carbon n ", nat(elnum(3))
c      print *, "name ", elnam(elnum(3))

      total_h = nat(elnum(1))+nmol(446)+nmol(447)+nmol(15)+nmol(1)
c      print *, "total H ", total_h
