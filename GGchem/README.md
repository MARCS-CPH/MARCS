...
> **Note:** this is a copy of GGchem vendored into the
> [MARCS-CPH/MARCS](https://github.com/MARCS-CPH/MARCS) repository so
> `marcs.f` can build and run it directly (see `src16/main_marcs.f`, the
> MARCS-specific driver that additionally writes `pp.dat`/`ndensity.dat`/
> `GGchem_ppel` for MARCS to read back). The example/tooling directories
> from the original distribution (`tools/`, `results/`, `structures/`,
> `input/`) are intentionally not carried here — get the full project,
> including those, at [pw31/GGchem](https://github.com/pw31/GGchem).
