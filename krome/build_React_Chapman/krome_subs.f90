...
contains
  ...
  !  KROME 14.08.dev on 2024-11-13 11:03:48
  !  Changeset xxxxxxx
  ...
  function coe(n)
    ...
    k(1) = small + (2.21E-34&
        *(Tgas&
        /298)**(-1.4)*exp(1.8/(8.314E-3*Tgas)))

    !O3 + O -> O2 + O2
    k(2) = small + (4.98E-12&
        *(Tgas&
        /298)**(0.5)*exp(-15.89/(8.314E-3*Tgas)))

    !O2 -> O + O
    k(3) = small + (0.00000000017059045154)

    !O3 -> O2 + O
    k(4) = small + (0.0181501903017985)

    !O3 + M -> O2 + O + M (REV)
    if(Tgas.GE.2.73d0 .and. Tgas.LT.1.d8) then
      k(5) = small + ((2.21E-34&
          *(Tgas&
          /298)**(-1.4)*exp(1.8&
          /(8.314E-3*Tgas))) &
          * revKc(Tgas,(/idx_O3,idx_M/),(/idx_O2,idx_O,idx_M/)) &
          * (1.3806488d-22 * Tgas)**(-1))
    end if
    ...
    if(Tgas.GE.2.73d0 .and. Tgas.LT.1.d8) then
      k(6) = small + ((4.98E-12&
          *(Tgas&
          /298)**(0.5)*exp(-15.89&
          /(8.314E-3*Tgas))) &
          * revKc(Tgas,(/idx_O2,idx_O2/),(/idx_O3,idx_O/)))
    end if

    !O + O -> O2 (REV)
    if(Tgas.GE.2.73d0 .and. Tgas.LT.1.d8) then
      k(7) = small + ((0.00000000017059045154) &
          * revKc(Tgas,(/idx_O,idx_O/),(/idx_O2/)) &
          * (1.3806488d-22 * Tgas)**(1))
    end if

    !O2 + O -> O3 (REV)
    if(Tgas.GE.2.73d0 .and. Tgas.LT.1.d8) then
      k(8) = small + ((0.0181501903017985) &
          * revKc(Tgas,(/idx_O2,idx_O/),(/idx_O3/)) &
          * (1.3806488d-22 * Tgas)**(1))
    end if
  ...
  function revHS(Tgas,idx)
    ...
    real*8::p1_nasa(8,7), p2_nasa(8,7), Tlim_nasa(8,3), p(7)
    real*8::p1_nist(8,7), p2_nist(8,7), Tlim_nist(8,3)
    integer::idx
  ...
  subroutine load_arrays()
    ...
    arr_r1(1:8) = (/1,4,1,4,4,1,2,1/)
    arr_r2(1:8) = (/2,2,8,8&
        ,3,1,2,2/)
    arr_r3(1:8) = (/3,8,8,8,8,8,8&
        ,8/)
    arr_p1(1:8) = (/4,1,2,1,1,4,1&
        ,4/)
    arr_p2(1:8) = (/3,1,2,2,2,2,8&
        ,8/)
    arr_p3(1:8) = (/8,8,8,8,3,8,8,8/)
