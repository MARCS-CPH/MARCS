...
contains
  ...
  !  KROME 14.08.dev on 2022-10-03 09:00:03
  !  Changeset ed8d3da
  ...
  function coe(n)
    ...
    !N2 + O -> NO + N
    k(1) = small + (3d-10*exp(-38300.*invT))

    !O2 + N -> NO + O
    k(2) = small + (1.5d-14*Tgas*exp(-3270.&
        *invT))
    ...
    if(Tgas.GE.2.73d0 .and. Tgas.LT.1.d8) then
      k(3) = small + ((3d-10&
          *exp(-38300.*invT)) &
          * revKc(Tgas,(/idx_NO,idx_N/),(/idx_N2,idx_O/)))
    end if
    ...
    if(Tgas.GE.2.73d0 .and. Tgas.LT.1.d8) then
      k(4) = small + ((1.5d-14&
          *Tgas*exp(-3270.*invT)) &
          * revKc(Tgas,(/idx_NO,idx_O/),(/idx_O2,idx_N/)))
    end if
  ...
  function revHS(Tgas,idx)
    ...
    real*8::p1_nasa(9,7), p2_nasa(9,7), Tlim_nasa(9,3), p(7)
    real*8::p1_nist(9,7), p2_nist(9,7), Tlim_nist(9,3)
    integer::idx
